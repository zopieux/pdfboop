      ...
      for (const [_, ref] of xObjects.entries()) {
        ...
        if (subtype === '/Image' || subtype === 'Image') {
          ...
          if (!seenRefs.has(refStr)) {
            seenRefs.add(refStr);
            const width = subDict.get(PDFName.of('Width'))?.asNumber() || subDict.get(PDFName.of('W'))?.asNumber() || 0;
            const height = subDict.get(PDFName.of('Height'))?.asNumber() || subDict.get(PDFName.of('H'))?.asNumber() || 0;
 ...
 */
function cloneObject(srcDoc: PDFDocument, dstDoc: PDFDocument, srcRef: PDFRef, refMap: Map<string, PDFRef>): PDFRef {
 ...
 */
export const extractAssetPreview = async (blob: Blob, refStr: string, limitScale?: number): Promise<string> => {
  ...
  try {
    ...
    const srcRef = PDFRef.of(parseInt(parts[0]), parseInt(parts[1]));
    const assetObj = srcDoc.context.lookup(srcRef);
    if (!(assetObj instanceof PDFStream)) return '';

    const width = assetObj.dict.get(PDFName.of('Width'))?.asNumber() || assetObj.dict.get(PDFName.of('W'))?.asNumber() || 100;
    const height = assetObj.dict.get(PDFName.of('Height'))?.asNumber() || assetObj.dict.get(PDFName.of('H'))?.asNumber() || 100;
    ...
    page.node.set(PDFName.of('Resources'), tempDoc.context.obj({
